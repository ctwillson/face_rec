	...
	vector<Mat> &testimages, vector<int> &testlabels)
{
	...
	string cur_dir;
	char *id[5]={"chenan","liwen","yuanquan"};
	int i,j;
	for(int i=0; i<K; i++)
	{
		cur_dir = dir;
		...
		for (j=0;j<file_vec.size();j++)
		{
			...
			//openCV中，Mat类型与CvMat和IplImage类型都可以代表和显示图像，但是，Mat类型侧重于计算，数学性较高，openCV对Mat类型的计算也进行了优化。而CvMat和IplImage类型更侧重于“图像”，openCV对其中的图像操作（缩放、单通道提取、图像阈值操作等）进行了优化。
...
void CaptureandRecognize(Ptr<FaceRecognizer>model)
{
	...
	char *strlabel[5]={"error","chenan","liwen","yuanquan"}; 
	...
	while(true)
	{
		...
		if(ori_detected &&strlen(ori_detected->imageData)>10)
		{
			...
			cvPutText(iplImg,strlabel[pred_label],cvPoint(r.x+r.width,r.y-20),&font,CV_RGB(255,0,0));//在图片中输出字符
